...
console.log(
  `Ignoring incidents from ${ignoreDays} days ago (${ignoreTime} ms).`
);
...
function buildIncidentEmbed(incident: any): MessageEmbed {
  ...
  while (
    embed.fields.map((f) => f.value.length).reduce((a, b) => a + b, 0) > 6000
  ) {
    embed.fields.pop();
  }
...
async function createMessage(incident: any) {
  ...
  json[id] = messageId;
  console.log(
    `Created new message for incident ${id} with message-id ${messageId}.`
  );
